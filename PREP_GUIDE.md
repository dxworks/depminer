...
If your project is only in the first row, you are done. The rest of this file is for the four
...
`dotnet restore` **is not enough.** A plain restore writes `obj/project.assets.json`, and
**neither scanner reads that file** — it yields 0 components in both Trivy and Syft. The only .NET
file either tool reads that carries a transitive graph is `packages.lock.json`, and that is
opt-in:
...
|---|---|---|
| A **lock file** (Gradle, .NET, npm, Python-lock, etc.) | a file **inside the repo** | ✅ Yes — prep anywhere, the file ships with the code |

...
* **Full transitive Maven trees offline.** Syft resolves from the local `~/.m2` cache via `SYFT_JAVA_RESOLVE_TRANSITIVE_DEPENDENCIES` and `SYFT_JAVA_USE_MAVEN_LOCAL_REPOSITORY` (both overridable, network stays off). On a large multi-module Maven project this multiplies the detected components several times over, with a warm cache and no network.
